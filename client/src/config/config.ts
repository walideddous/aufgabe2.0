...
export const GET_STOP_SEQUENCE_BY_MODES = (modes: string ) => {
  return `query {stopSequenceByMode(modes:"${modes}"){_id,name,modes,date,schedule{day,time{start,end}},stopSequence{_id,name,modes,coord{WGS84{lat,lon}}}}}`
}

export const GET_ALL_STOP_SEQUENCE = `query {stopSequence{_id,name,date,modes,schedule{day,time{start,end}},stopSequence{_id,name,modes,coord{WGS84{lat,lon}}}}}`

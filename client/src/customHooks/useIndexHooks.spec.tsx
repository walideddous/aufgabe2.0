...
describe('Test the customHooks of the /components/index.tsx', () => {
  ...
  it('Should return an array of stops when we choose the mode', async () => {
    ...
    expect(result.current.stops.length).toEqual(1);
  });
});
...
describe("Test the customHooks of the /components/index.tsx", () => {
  ...
  it("Should store the data in local if we send a request to the Backend", async () => {
    ...
    expect(result.current.managedRoutes.length).toBe(1);
  });
  ...
  it("Should display stopSequence when we trigger the handleLoadManagedRouteQuery  function", async () => {
    const { result, waitForNextUpdate } = renderHook(() => useIndexHooks());
    ...
    expect(result.current.managedRoutes.length).toBe(1);
    act(() => {
      result.current.handleLoadManagedRouteQuery("St. Gallen to Zürich HB");
    });

    expect(result.current.currentManagedRoute).not.toBeNull();
  });
  it("Should select the stop and calculate the distance when we trigger the clickOnDrop function", async () => {
    ...
    expect(result.current.selectedStop?.name).toBe("Lyon");
  });
  ...
  it("Should add stop on stopSequence field when we trigger the handleAddStopsOnCLick  function", async () => {
    ...
    expect(
      result.current.stopSequence.trajekt.items.filter((el) => el.name === "Lyon")
        .length
    ).toBeTruthy();
  });
  ...
  it("Should delete the stop Sequence when we trigger the handleDeleteStopSequenceMutation function", async () => {
    ...
    const beforeDeleteStopSequenceLength =
      result.current.managedRoutes.length;
    ...
    expect(result.current.managedRoutes.length).toEqual(
  ...
  it("Should check the handleAddAfterselectedStop function", async () => {
    const { result, waitForNextUpdate } = renderHook(() => useIndexHooks());
    ...
    act(() => {
      result.current.handleAddAfterselectedStop("Lyon");
    });

    expect(result.current.stopSequence.trajekt.items[1].name).toBe("Lyon");
  });
  it("Should check the handleAddBeforeselectedStop function", async () => {
    const { result, waitForNextUpdate } = renderHook(() => useIndexHooks());
    ...
    act(() => {
      result.current.handleAddBeforselectedStop("Lyon");
    });

    expect(result.current.stopSequence.trajekt.items[0].name).toBe("Lyon");
  });
  ...
  it("Should check the handleDeleteMarkerFromMap function", async () => {
    ...
    expect(result.current.stopSequence.trajekt.items.length).toBe(0);

    act(() => {
      result.current.handleSelectAutoSearch("Basel");
    });

    expect(result.current.stopSequence.trajekt.items.length).toBe(1);
  ...
  it("Should check the handleDeleteMarkerFromMap function", async () => {
    ...
    expect(result.current.stopSequence.trajekt.items.length).toBe(2);
    ...
    expect(result.current.selectedStop?.name).toBe("Basel");
  });
